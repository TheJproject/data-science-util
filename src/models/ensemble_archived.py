...
def main(cfg: DictConfig) -> None:
    ...
    train_prepared_path = os.path.join(data_dir, 'processed/{cfg.competition_name}/train_prepared.feather')
    train_df_prepared = feather.read_feather(train_prepared_path)
    test_holdout_path = os.path.join(data_dir, 'processed/{cfg.competition_name}/test_holdout_prepared.feather')
    ...
    for name in cfg.ensemble_list:
        param_path = os.path.join(model_dir, f'{cfg.competition_name}/models_{cfg.selected_model}/{name}_param.pkl')
